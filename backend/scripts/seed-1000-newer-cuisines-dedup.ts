// backend/scripts/seed-1000-newer-cuisines-dedup.ts
//
// 1000-recipe duplicate-safe seed. Byte-identical to
// seed-500-newer-cuisines-dedup.ts (same dedup guard, retry-on-dup,
// diversity axis, TTY-aware progress) — the ONLY difference is the default
// RECIPE_BUDGET (1000 instead of 500). The per-cuisine floors in
// CUISINE_TARGETS (118 each across ~70 cuisines) already provide far more
// than 1000 of planned headroom, so a bigger budget simply lets more of
// that plan through; nothing else changes. Override with RECIPE_BUDGET=…
// for any other size.
//
// The dedup guard skips persisting a recipe whose normalized title
// (src/utils/recipeTitleKey.ts) already exists in the DB or was generated
// earlier in this same run, so a 1000-run on top of an existing catalog
// doesn't stack a third "Argentinian Chimichurri Steak".
//
// Tier U launch seed run — 1000 recipes to fill out the cuisine taxonomy
// gaps exposed by the region-grouped filter (frontend/utils/cuisineTaxonomy.ts).
//
...
//   groq           Llama 3.3 70B. ~$3 for 1000. Fastest (~250 tok/s,
//                  ~200 min total). Requires GROQ_API_KEY (no ID verification).
//   deepseek       Default. DeepSeek-V3. ~$3.60 for 1000. Slightly stronger structured
//                  JSON than Llama. Requires DEEPSEEK_API_KEY.
//   gemini         ~$1.30 for 1000 with Flash-Lite. Requires GOOGLE_AI_API_KEY
//                  (note: Google may require account-ID verification).
//   claude         ~$16 for 1000 with Haiku 4.5, ~$120 with the legacy Sonnet
...
//                  Set OLLAMA_MODEL (default llama3.1:70b). ~14 hrs for 1000.
//
...
//
// Usage examples:
//   GROQ_API_KEY=... DRY_RUN=1 npx ts-node scripts/seed-1000-newer-cuisines-dedup.ts
//   GROQ_API_KEY=... DEEPSEEK_API_KEY=... DRY_RUN=0 npx ts-node scripts/seed-1000-newer-cuisines-dedup.ts
...
const RECIPE_BUDGET = Number(process.env.RECIPE_BUDGET ?? 1000);
...
// failure. Tradeoff at high concurrency: staler avoid-context → slightly more
// dup-retries. Start ~5.
...
async function main(): Promise<void> {
  console.log('▶ Tier U seed run — 1000 recipes across newer / thin cuisines');
  console.log('');
    ...
    while (cursor < plan.length) {
      ...
      for (let attempt = 0; attempt <= DUP_RETRIES && !slotDone; attempt += 1) {
        try {
          ...
          // Reserve the title key SYNCHRONOUSLY before the save await so a
          // concurrent worker can't pass the has()-check for the same title
          // mid-save. Rolled back if the save throws.
          ...
          if (titleKey) {
            // Immutable re-read so a peer worker's append isn't clobbered.
      ...
      // apiCalls (not i) drives the bar — robust to out-of-order completion.
      renderProgress(
        Math.min(apiCalls, plan.length),
        plan.length,
        { saved: succeeded, dup: skippedDuplicate, fail: failed },
        label,
        { final: apiCalls >= plan.length },
      );
    }
  }
