...
//
// Per-month produce-peak table for the Northern Hemisphere temperate
// growing zone (the user base's center of gravity at v1). Each month
// has 3–6 ingredients at peak. The Today card picks one per day using
// a deterministic day-of-month index so users see a rotation without
// repeats inside a single week.
...
export const SEASONAL_PRODUCE_BY_MONTH: ReadonlyArray<readonly SeasonalProduce[]> = [
  // 0 = January
  [
...
/**
 * Pick one peak ingredient for the day. Deterministic on (year, month, dayOfMonth)
 * so the same calendar day shows the same suggestion regardless of when the
 * Today screen mounts. Different days inside one week always pick different
...
export function pickSeasonalProduce(input: SeasonalPickInput = {}): SeasonalProduce | null {
  ...
  // The day-of-month modulo length gives us a clean rotation. Since most months
  // have 4–5 entries, every 4–5 days the cycle wraps — fine for a daily card.
  const idx = (day - 1) % list.length;
  return list[idx];
}

/**
 * Find the month index where a given ingredient is at peak. Returns -1 if it's
 * not on the seasonal list at all. Used by the future "save for the season"
 * Coach prompt — let Sazon say "asparagus is in march–may, save the recipe
 * for then" instead of suggesting a recipe that's out of season.
 */
export function findPeakMonth(ingredientName: string): number {
  const target = ingredientName.trim().toLowerCase();
  if (!target) return -1;
  for (let m = 0; m < SEASONAL_PRODUCE_BY_MONTH.length; m += 1) {
    if (SEASONAL_PRODUCE_BY_MONTH[m].some((p) => p.name === target)) {
      return m;
    }
  }
  return -1;
